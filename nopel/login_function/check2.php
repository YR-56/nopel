...
if (!empty($_POST['check'])) {
    // パスワードを暗号化
    $hash = hash("sha256", $_SESSION['join']['password']);

    // 入力情報をデータベースに登録
    $statement = $db->prepare("INSERT INTO members SET name=?, memberID=?, password=?");
    $statement->execute(array(
        
        $_SESSION['join']['name'],
        $_SESSION['join']['memberID'],
        $hash
...
<body>
    <div class="content">
        <form action="" method="POST">
            ...
            <div class="control">
                <p>会員ID</p>
                <p><span class="fas fa-angle-double-right"></span> <span class="check-info"><?php echo htmlspecialchars($_SESSION['join']['memberID'], ENT_QUOTES); ?></span></p>
            </div>
